...
class GetLightcurve:
    def __init__(self,
        ...
        # Get light curve from a folder with fits files
        if fits_files_folder is not None:
            self._fits_files_folder = fits_files_folder
        ...
        if target is not None and lc_file is None and fits_files_folder is None:
            self.set_lightcurve_info(author=author,
                                     cadence=cadence,
                                     sector=sector,
                                     quarter=quarter,
                                     mission=mission)
        ...
        if target is None and lc_file is None and fits_files_folder is None:
    ...
    def lightcurve_from_file(self):
        ...
        data = np.genfromtxt(self._lc_file, delimiter=',', names=['time', 'flux', 'flux_err'])
    ...
    def lightcurve_from_target_name(self):
        ...
        lc = search_results.download_all(quality_bitmask='default').stitch().remove_nans().remove_outliers(5)
        self._time = lc.time.value
        self._flux = lc.flux.value
        self._flux_err = lc.flux_err.value
        return self
