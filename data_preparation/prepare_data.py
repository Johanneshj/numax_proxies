...
def get_lightcurve(*args, **kwargs):
    ...
    # Case 3: JSON file only
        ...
        for lc in lc_collection:
            ...
            flux = (flux - 1) * 1e6
            lc = lk.LightCurve(
                time=lc.time.value, flux=flux, flux_err=lc.flux_err.value
            )
            lc_list.append(lc)
    ...
    raise ValueError(
        "Invalid arguments. Provide either an ID string or time+flux arrays."
    )
...
def prepare_lightcurve(lc=None, id=None, *args, **kwargs):
    ...
    # print(time)
    original_flux += (
        calculate_noise(original_flux, kwargs.get("noise_std", None))
        if kwargs.get("add_noise", None)
        else 0
    ...
    if savgol:
        ...
        if plot:
            plot_lc(time=lc.time.value, flux=lc.flux.value, id=id)
        return lc, dt
...
def find_author(search_results):
    ...
    author = sorted(
        most_common,
        key=lambda x: (
            priority.index(x[0]) if x[0] in priority else len(priority),
            -x[1],
        ),
    )[0][0]
    return author
    ...
    if filters is not None:
        ...
        for i in range(n_filters):
            ...
            axs[0].scatter(
                ...
                s=2,
                zorder=i + 1,
                label=f"smoothing iteration {int(i)}",
            )
