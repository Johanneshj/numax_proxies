...
class DataProcessing:
    ...
    def close_gaps(self):
        ...
        gap_indices = np.concatenate((np.where(np.diff(time) > gap_size_secs)[0], [len(time)-1]))
    ...
    def averaged_psd(self):
        ...
        time     = time[:size_chunk*n_chunk].reshape((n_chunk, size_chunk)) * 86400 # convert back to seconds
        flux     = flux[:size_chunk*n_chunk].reshape((n_chunk, size_chunk))
        flux_err = flux_err[:size_chunk*n_chunk].reshape((n_chunk, size_chunk))

        freq, psd = self.calculate_psd_for_avg_psd(time[0], flux[0], flux_err[0], freq_grid=None)
        
        psd = [self.calculate_psd_for_avg_psd(t_chunk, f_chunk, ferr_chunk, freq_grid=freq) for t_chunk, f_chunk, ferr_chunk in zip(time, flux, flux_err)]
        psd = sum(psd)
    ...
    def windowfunction(self, df, width=None, oversampling=10):
        ...
        x = 0.5*np.sin(2*np.pi*freq_cen*self._ls.t) + 0.5*np.cos(2*np.pi*freq_cen*self._ls.t)
