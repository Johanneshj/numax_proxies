...
def query_gaia(id=None, ra=None, dec=None):
    ...
    res = Simbad.query_objectids(object_name=id)
    if len(res) < 1:
        return ValueError('no objects with that id')
    else:
        Gaia_ids = [res[i][0] for i in range(len(res)) if 'gaia' in res[i][0].lower()]
        if len(res) < 1:
            return ValueError('no gaia ids available')
        else:
            gaia_id = np.sort(Gaia_ids)[-1]
