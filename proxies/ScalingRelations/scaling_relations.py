...
def compute_numaxes(*args, **kwargs):
    if kwargs.get('numax_dictionary'):
        numax_dict = kwargs.get('numax_dictionary')
    else:
    ...
    for param in params:
        if kwargs.get(f'{param}'):
            numax_dict.setdefault(f'{param}',
                                ufloat(kwargs.get(f'{param}', np.nan),
    ...
    numaxes = numax_scaling_relations(logg=loggs,
                                      teff=teffs,
                                      lum=lums,
                                      mass=masses,
                                      radius=radii)    
...
def numax_scaling_relations(logg=None, teff=None, lum=None, mass=None, radius=None):
    """
...
def make_broadcastable_uarray(param, dictionary):
    ...
    vals = [entry[1] for entry in numax_dict.items() if f'{param}' in entry[0] and entry[1] is not np.nan]
    arr = unp.uarray(
                    [[val.nominal_value] for val in vals],
                    [[val.std_dev] for val in vals]
                    )
    return arr
