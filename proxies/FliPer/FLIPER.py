...
class FLIPER:
    ...
    def Fp_20_days(self, star_tab_psd_20, kepmag):
        ...
        end_20 = np.amax(DATA_PREPARATION().GET_ARRAY(star_tab_psd_20)[0])
        ...
        sig_Fp07 = self.Fp_error(
            DATA_PREPARATION().CUT_SPECTRA(star_tab_psd_20, 0.7, end_20)
        )
        sig_Fp7 = self.Fp_error(
            DATA_PREPARATION().CUT_SPECTRA(star_tab_psd_20, 7, end_20)
        )
        sig_Fp20 = self.Fp_error(
            DATA_PREPARATION().CUT_SPECTRA(star_tab_psd_20, 20, end_20)
        )
        sig_Fp50 = self.Fp_error(
            DATA_PREPARATION().CUT_SPECTRA(star_tab_psd_20, 50, end_20)
        )
    ...
    def Fp_80_days(self, star_tab_psd_80, kepmag):
        ...
        end_80 = np.amax(DATA_PREPARATION().GET_ARRAY(star_tab_psd_80)[0])
        ...
        sig_Fp02 = self.Fp_error(
            DATA_PREPARATION().CUT_SPECTRA(star_tab_psd_80, 0.2, end_80)
        )
    ...
    def Fp_error(self, power):  # GUY
        ...
        Ptmp = np.array(
            [np.sum(power[i * n : (i + 1) * n]) for i in range(int(len(power) / n))]
        )  #   power on the rebin
        Ptot = np.sum(Ptmp)  #   total power
        sig_Ptot = (
            np.sum((2 * Ptmp / 2 / n * n**0.5) ** 2)
        ) ** 0.5  #   uncertainties on total power
...
class DATA_PREPARATION:
    ...
    def MAG_COR_KEP(self, star_tab_psd, kepmag):
        ...
        c = 3.46 * 10 ** (0.4 * (12.0 - kepmag) + 8)
        siglower = np.sqrt(c + 7e6 * max([1, kepmag / 14.0]) ** 4.0) / c
        siglower = siglower * 1e6  #  [ppm]
        dt = 1.0 / (2 * 0.000278)  #  [sec]
        siglower = siglower**2.0 * 2 * dt * 1.0e-6  #  [ppm^2/muHz]
        return siglower
    ...
    def REGION(self, star_tab_psd, inic, end):
        ...
        """
        x = np.float64(self.GET_ARRAY(star_tab_psd)[0])  # convert frequencies in muHz
    ...
    def APODIZATION(self, star_tab_psd):
        ...
        freq = star_tab_psd[:, 0]
        nq = max(freq)
        nu = np.sin(np.pi / 2.0 * freq / nq) / (np.pi / 2.0 * freq / nq)
        power = power / (nu**2)
...
class ML:
    ...
    def PREDICTION(
        self, F02, F07, F7, F20, F50, noise, cadence, mission, path_to_training_file
    ):
        """
        ...
        listing = [
            {
                "lnF02": self.CONVERT_TO_LOG(F02),
                "lnF07": self.CONVERT_TO_LOG(F07),
                "lnF7": self.CONVERT_TO_LOG(F7),
                "lnF20": self.CONVERT_TO_LOG(F20),
                "lnF50": self.CONVERT_TO_LOG(F50),
                "noise": noise,
                "cadence": cadence,
                "mission": mission,
            }
        ...
        rf = joblib.load(path_to_training_file)
        prediction = rf.predict(X)
        return prediction[0]

    def CONVERT_TO_LOG(self, param):
        if param <= 0:  # Safety, FliPer still works with NaNs?
            return np.nan
